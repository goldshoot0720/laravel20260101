...
class SubscriptionOriginal extends BaseModel {
    ...
    // 獲取所有訂閱，按到期日期排序
    public function getAllSubscriptions($orderBy = 'todate ASC', $limit = null) {
        return $this->all([], $orderBy, $limit);
    }
    
    // 搜尋訂閱
    public function searchSubscriptions($query, $limit = null) {
        return $this->search($query, ['name', 'url', 'note'], [], 'todate ASC', $limit);
    }
    ...
    public function getStatistics() {
        ...
        // 月度總支出（假設都是月付）
        $sql = "SELECT SUM(price) as monthly_cost FROM {$this->table} WHERE todate >= CURDATE()";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
    ...
    public function createSubscription($data) {
        ...
        if (isset($data['website_url'])) $mappedData['url'] = $data['website_url'];
        if (isset($data['price'])) $mappedData['price'] = $data['price'];
        if (isset($data['next_payment_date'])) $mappedData['todate'] = $data['next_payment_date'];
